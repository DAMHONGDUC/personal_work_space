/**
 * Generates cv/build/ from the CV data file (see resource-constant.mts).
 *
...
const dataFile = path.join(root, ResourceConstant.CV_DATA_FILE);
...
const outDir = path.join(root, ResourceConstant.CV_BUILD_DIR);
...
if (!fs.existsSync(photo)) {
  throw new Error(
    `${ResourceConstant.CV_DATA_FILE} points at header.photo "${cv.header.photo}", which is not in cv/assets.`,
  );
}
